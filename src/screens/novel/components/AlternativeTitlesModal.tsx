...
const AlternativeTitlesModal: React.FC<AlternativeTitlesModalProps> = ({
  ...
  const loadTitles = useCallback(async () => {
    try {
      ...
      showToast(
        `Failed to load alternative titles: ${
  ...
  const handleAddTitle = async () => {
    ...
    try {
      ...
      setNewTitle('');
      await loadTitles();
      showToast('Alternative title added');
    } catch (error) {
      showToast(
        `Failed to add alternative title: ${
          error instanceof Error ? error.message : String(error)
        }`,
      );
    }
          ...
          { text: 'Cancel', style: 'cancel' },
          {
            ...
            onPress: async () => {
              try {
                ...
                await loadTitles();
                showToast('Alternative title deleted');
              } catch (error) {
                showToast('Failed to delete alternative title');
              }
            },
          },
        ],
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setEditingText('');
      await loadTitles();
      showToast('Alternative title updated');
    } catch (error) {
      showToast('Failed to update alternative title');
    }
  };
  ...
  const handleClearAll = () => {
    Alert.alert(
      ...
      'Are you sure you want to delete all alternative titles?',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Clear All',
          ...
          onPress: async () => {
            try {
              ...
              await loadTitles();
              showToast('All alternative titles cleared');
            } catch (error) {
              showToast('Failed to clear alternative titles');
            }
          },
        },
      ],
      ...
      >
        <View style={styles.content}>
          <Appbar.Header style={styles.header}>
            ...
            <Appbar.Content title="Alternative Titles" />
            {titles.length > 0 && (
          ...
          <View style={styles.addSection}>
            <TextInput
              label="Add new alternative title"
              value={newTitle}
